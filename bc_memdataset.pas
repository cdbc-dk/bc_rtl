...
type
  ...
  protected
    ...
    { 11.05.2012 bc - mostly stolen from TDbf }
    function LocateRecord(const KeyField: string;KeyValue: string;Options: TLocateOptions): Boolean;
    // Own.
  ...
  public
    ...
    function Locate(const KeyFields: string; const KeyValues: string; Options: TLocateOptions): Boolean; overload;
...
begin
  L:= ReadInteger(S);
  Setlength(Result,L);
  if (L<>0) then S.ReadBuffer(Result[1],L);
end;
...
begin
  ...
  if (L<>0) then S.WriteBuffer(Value[1],L);
end;
...
begin
  ...
  case dt1 of
    ...
    ftDateTime,
    ftTime,ftDate: result:= SizeOf(TDateTime);
  else
...
begin
  case State of
    dsBrowse: if IsEmpty then Buffer:= nil else Buffer:= ActiveBuffer;
    dsEdit,
...
begin
  ...
  try
    if FCurrRecNo>0 then begin
      ...
      end else TS.CopyFrom(FStream,MDSGetRecordOffset(FRecCount-1));
    end else begin                                  //Delete first Rec
      FStream.Position:= MDSGetRecordOffset(FCurrRecNo+1);
      TS.CopyFrom(FStream,(MDSGetRecordOffset(FRecCount))-MDSGetRecordOffset(FCurrRecNo+1));
    end;
    FStream.loadFromStream(TS);
    Dec(FRecCount);
    if FRecCount=0 then FCurrRecNo:= -1 else
...
begin
  if (FOpenStream<>Nil) then ReadFieldDefsFromStream(FOpenStream);
end;
...
begin
  P:= F.Position;
  if F.Read(I,MarkerSize)<>MarkerSize then RaiseError(SErrInvalidDataStream,[P])
  else if (I<>Marker) then RaiseError(SErrInvalidMarkerAtPos,[P,I,Marker]);
end;
...
begin
  ...
  if (FFileName<>'') then FOpenStream:=TFileStream.Create(FFileName,fmOpenRead);
  try
    InternalInitFieldDefs;
    if DefaultFields then CreateFields;
    BindFields(True);
    FCurrRecNo:= -1;
    if (FOpenStream<>nil) then begin
      LoadDataFromStream(FOpenStream);
      CheckMarker(FOpenStream,smEOF);
    end;
  finally FreeAndNil(FOpenStream); end;
  FIsOpen:= True;
end;
...
begin
  ...
  try LoadFromStream(F); finally F.Free; end;
end;
...
Var
  F : TFileStream;
begin
  if (AFileName='') then RaiseError(SErrNoFileName,[]);
  F:= TFileStream.Create(AFileName,fmCreate);
  try SaveToStream(F,SaveData); finally F.Free; end;
end;
...
begin
  if (FFileModified) and (FFileName<>'') then SaveToFile(FFileName,True);
  FIsOpen:= false;
  FFileModified:= false;
  // BindFields(False);
...
begin
  ...
  if (State=dsEdit) then MDSWriteRecord(ActiveBuffer, FCurrRecNo)
...
begin
  ...
  repeat
    case GetMode of
      gmCurrent: if (FCurrRecNo>=FRecCount) or (FCurrRecNo<0) then Result:= grError;
      gmNext: if (FCurrRecNo<FRecCount-1) then Inc(FCurrRecNo) else Result:= grEOF;
      gmPrior: if (FCurrRecNo>0) then Dec(FCurrRecNo) else result:= grBOF;
    end;
    if result = grOK then begin
      ...
      if (Filtered) then Accepted:=MDSFilterRecord(Buffer) //Filtering
      else Accepted:= True;
      if (GetMode=gmCurrent) and not Accepted then result:= grError;
    end;
...
begin
  I:= Field.FieldNo - 1;
  result:= (I >= 0) and MDSGetActiveBuffer(SrcBuffer) and not getfieldisnull(pointer(srcbuffer),I);
...
begin
  I:= Field.FieldNo - 1;
  if (I >= 0) and  MDSGetActiveBuffer(DestBuffer) then begin
    if State in [dsEdit, dsInsert, dsNewValue] then Field.Validate(Buffer);
    if buffer = nil then setfieldisnull(pointer(destbuffer),I) else begin
      unsetfieldisnull(pointer(destbuffer),I);
      J:=GetIntegerPointer(FFieldSizes, I)^;
      if Field.DataType=ftString then Dec(J); // Do not move terminating 0, which is in the size.
...
begin
  ...
  else RaiseError(SErrBookMarkNotFound,[ReqBookmark]);
end;
...
begin
  ...
  Finally RestoreState(SaveState); end;
end;
...
begin
  CheckBrowseMode;
  if (Value>=1) and (Value<=FRecCount) then begin
    FCurrRecNo:= Value-1;
    Resync([]);
  end;
end;
...
begin
  UpdateCursorPos;
  if (FCurrRecNo<0) then Result:= 1 else Result:= FCurrRecNo+1;
end;
...
function TMemDataset.LocateRecord(const KeyField: string;KeyValue: string;Options: TLocateOptions): Boolean;
begin
  Result:= false;
  KeyValue:= LowerCase(KeyValue); { caseinsensitive }
  Self.First;
  while not Self.EOF do begin
    if LowerCase(Self.FieldByName(KeyField).AsString) = KeyValue then begin
      fLocateRecNo:= FCurrRecNo;
      Result:= true;
      break;
    end;
    Self.Next;
...
function TMemDataset.Locate(const KeyFields: string; const KeyValues: string; Options: TLocateOptions): Boolean;
var
  saveRecNo: integer;
begin
  saveRecNo:= GetRecNo;
  ...
  if Result then begin
    if fLocateRecNo <> -1 then SetRecNo(fLocateRecNo+1);
    DoAfterScroll;
  end else SetRecNo(saveRecNo);
end; { 11.05.2012 bc - mostly stolen from TDbf }
