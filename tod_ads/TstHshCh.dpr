...
(*------------------------------------------------------------------*)
(* TstHshCh                                                         *)
(* Hash table (chaining) test suite                                 *)
(********************************************************************)
...
var
  i, j : integer;
  HashTable : TtdHashTableChained;
  StrList : TStringList;
  DummyItem : pointer;
begin
  writeln('Testing hash table with chaining');
  try
    StrList := TStringList.Create;
    HashTable := TtdHashTableChained.Create(53, TDPJWHash, nil);
    try
      HashTable.MaxLoadFactor := 20;
