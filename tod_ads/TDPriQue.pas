...
type
  ...
  TtdPriorityQueueEx = class
    ...
    protected
      ...
      {$ENDIF}
    public
      constructor Create(aCompare : TtdCompareFunc;            {!!.01}
                         aDispose : TtdDisposeProc);           {!!.01}
      destructor Destroy; override;
...
{--------}
procedure ClearLinkedList(aLinkedList : PpqexNode;             {!!.01}
                          aDispose    : TtdDisposeProc);       {!!.01}
var
...
begin
  ...
  while (Walker^.peNext <> nil) do begin
    if Assigned(aDispose) then                                 {!!.01}
      aDispose(Walker^.peItem);                                {!!.01}
    Temp := Walker;
...
{===TtdPriorityQueueEx===============================================}
constructor TtdPriorityQueueEx.Create(aCompare : TtdCompareFunc; {!!.01}
                                      aDispose : TtdDisposeProc);{!!.01}
begin
  inherited Create;
...
begin
  ClearLinkedList(FHandles, FDispose);                         {!!.01}
  FList.Clear;
end;
{--------}
